		...
		<section id="board_view">
			...
			<table>
				<tbody>
					...
					<tr>
						<td>
						<c:out value="글쓴이: ${board.id}" /> <c:choose>
					...
					<tr>
						<td>조회수: ${board.hit}&nbsp;&nbsp;&nbsp;&nbsp; <!-- 좋아요 영역 -->
							<c:choose>
								<c:when test="${empty sessionId}">
									<span><i class="fa-solid fa-heart" style="color: #000"></i></span>
									<span>${voteCount}</span>
									<a href="#" onclick="location.href='/loginform.do' ">(좋아요는
										로그인이 필요합니다.)</a>
								</c:when>
								<c:otherwise>
									<c:choose>
					...
					<tr>
						<td class="file"><c:if test="${sessionId eq board.id}">
			...
			<!-- 댓글 영역 -->
			<h3>
				<i class="fa-regular fa-pen-to-square"></i> 댓글
			</h3>
			...
			<c:if test="${empty sessionId}">
				<div class="replylogin">
					<a href="/loginform.do"> <i class="fa-solid fa-user"></i> 로그인한
						사용자만 댓글 등록이 가능합니다.
					</a>
				</div>
			</c:if>
